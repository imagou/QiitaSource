 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: target_inithook.c 698 2018-08-28 10:17:32Z roi $
 */
...
#define LSE_TIMEOUT_VALUE          (5000*1000)	/* 5 s    */
#define DBP_TIMEOUT_VALUE          (100*1000)	/* 100 ms */
...
#define PWR_REGULATOR_VOLTAGE_SCALE1_BOOST  0x00000000	/* Voltage scaling range 1 boost mode  */
...
PWRE_ControlVoltageScaling(uint32_t VoltageScaling)
{
	...
	if(VoltageScaling != PWR_REGULATOR_VOLTAGE_SCALE1
			&& VoltageScaling != PWR_REGULATOR_VOLTAGE_SCALE2 && VoltageScaling != PWR_REGULATOR_VOLTAGE_SCALE1_BOOST)
		return E_PAR;
	...
	if(VoltageScaling == PWR_REGULATOR_VOLTAGE_SCALE1_BOOST){
		...
		if((sil_rew_mem((uint32_t *)(TADR_PWR_BASE+TOFF_PWR_CR1)) & PWR_CR1_VOS) == PWR_REGULATOR_VOLTAGE_SCALE2){
			...
			sil_andw_mem((uint32_t *)(TADR_PWR_BASE+TOFF_PWR_CR5), PWR_CR5_R1MODE);

			/* Set Range 1 */
			sil_modw_mem((uint32_t *)(TADR_PWR_BASE+TOFF_PWR_CR1), PWR_CR1_VOS, PWR_REGULATOR_VOLTAGE_SCALE1);

			/* Wait until VOSF is cleared */
			tick = (PWR_FLAG_SETTING_DELAY_US * (4000000 / 1000000));
		...
		if((sil_rew_mem((uint32_t *)(TADR_PWR_BASE+TOFF_PWR_CR1)) & PWR_CR1_VOS) == PWR_REGULATOR_VOLTAGE_SCALE2){
			...
			sil_orw_mem((uint32_t *)(TADR_PWR_BASE+TOFF_PWR_CR5), PWR_CR5_R1MODE);
			/* Set Range 1 */
			sil_modw_mem((uint32_t *)(TADR_PWR_BASE+TOFF_PWR_CR1), PWR_CR1_VOS, PWR_REGULATOR_VOLTAGE_SCALE1);
		...
		/* Set Range 2 */
		sil_modw_mem((uint32_t *)(TADR_PWR_BASE+TOFF_PWR_CR1), PWR_CR1_VOS, PWR_REGULATOR_VOLTAGE_SCALE2);
...
RCC_SetFlashLatencyFromMSIRange(unsigned int msirange)
{
	...
	if(vos == PWR_CR1_VOS_0){		/* VOLTAGE_SCALE1 */
		...
		if(sysclk >= 16000000)		/* 16MHz以上 */
			latency = FLASH_ACR_LATENCY_2WS; /* 2WS */
		else{
			if(sysclk == 8000000)	/* 8MHz */
				latency = FLASH_ACR_LATENCY_1WS; /* 1WS */
			/* その他は 0WS */
		}
	}
...
SystemClock_Config(void)
{
	...
	if((ercd = PWRE_ControlVoltageScaling(PWR_REGULATOR_VOLTAGE_SCALE1_BOOST)) != E_OK)
		return ercd;
	 ...
	 */
	if(FLASH_ACR_LATENCY_3WS > (sil_rew_mem((uint32_t *)(TADR_FLASH_R_BASE+TOFF_FLASH_ACR)) & FLASH_ACR_LATENCY)){
		/*
		 *  LATENCYを設定し、設定確認
		 */
		sil_modw_mem((uint32_t *)(TADR_FLASH_R_BASE+TOFF_FLASH_ACR), FLASH_ACR_LATENCY, FLASH_ACR_LATENCY_3WS);
		if((sil_rew_mem((uint32_t *)(TADR_FLASH_R_BASE+TOFF_FLASH_ACR)) & FLASH_ACR_LATENCY) != FLASH_ACR_LATENCY_3WS){
			return E_SYS;
		}
	}
	 ...
	 *  HCLKの分周値設定
	 */
	sil_modw_mem((uint32_t *)(TADR_RCC_BASE+TOFF_RCC_CFGR), RCC_CFGR_HPRE, RCC_CFGR_HPRE_DIV2);
	 ...
	 *  要求のFLASH LATENCYが現状より高い場合、先にFLASHのLATENCY設定を行う
	 */
	if(FLASH_ACR_LATENCY_5WS > (sil_rew_mem((uint32_t *)(TADR_FLASH_R_BASE+TOFF_FLASH_ACR)) & FLASH_ACR_LATENCY)){
		/*
		 *  LATENCYを設定し、設定確認
		 */
		sil_modw_mem((uint32_t *)(TADR_FLASH_R_BASE+TOFF_FLASH_ACR), FLASH_ACR_LATENCY, FLASH_ACR_LATENCY_5WS);
...
SystemLowClock_Config(uint8_t msirange)
{
	...
	if((sil_rew_mem((uint32_t *)(TADR_RCC_BASE+TOFF_RCC_CR)) & RCC_CR_PLLON) != 0){
		/*
		 *  PLL停止
		 */
		sil_andw_mem((uint32_t *)(TADR_RCC_BASE+TOFF_RCC_CR), RCC_CR_PLLON);
...
{
	/* FPU settings ------------------------------------------------------------*/
	sil_orw_mem((uint32_t *)(TADR_SCB_BASE+TOFF_SCB_CPACR), ((3UL << 10*2)|(3UL << 11*2)));	/* set CP10 and CP11 Full Access */
	...
	sil_orw_mem((uint32_t *)(TADR_RCC_BASE+TOFF_RCC_APB1ENR1), RCC_APB1ENR1_PWREN);
	sil_orw_mem((uint32_t *)(TADR_PWR_BASE+TOFF_PWR_CR2), PWR_CR2_IOSV);	/* for LPUART */
