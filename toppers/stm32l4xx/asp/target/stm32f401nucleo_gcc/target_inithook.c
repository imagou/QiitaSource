 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: target_inithook.c 699 2018-08-07 21:24:34Z roi $
 */
...
ER sysclock_config(int mode)
{
	 ...
	 *  VOLTAGE設定
	 */
	sil_mdw_mem((uint32_t *)(TADR_PWR_BASE+TOFF_PWR_CR), (PWR_CR_VOS), PWR_REGULATOR_VOLTAGE_SCALE2);
	 ...
	 *  FLASH設定を2WSに設定
	 */
	sil_wrb_mem((uint8_t *)ACR_BYTE0_ADDRESS, FLASH_ACR_LATENCY_2WS);

	/*
	 *  FLASH設定が反映されなければエラー終了
	 */
    if((sil_rew_mem((uint32_t *)(TADR_FLASH_R_BASE+TOFF_FLASH_ACR)) & FLASH_ACR_LATENCY) != FLASH_ACR_LATENCY_2WS){
		return E_SYS;
    }
	 ...
	 *  PCLK1のコンフィギュレーション
	 */
	sil_mdw_mem((uint32_t *)(TADR_RCC_BASE+TOFF_RCC_CFGR), (RCC_CFGR_PPRE1), RCC_CFGR_PPRE1_DIV2);
